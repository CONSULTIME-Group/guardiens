 ...
 *
 * Objectif : indexabilite SEO des fiches gardien /gardiens/:id. Une fiche est
 * indexable si elle porte une bio d'au moins 80 caracteres ET au moins un
 * signal de confiance (identite verifiee OU au moins une photo de galerie).
 * 390 gardiens ont le texte mais aucun signal, leur fiche est donc en noindex.
 * Une seule photo de galerie suffit a la rendre indexable.
 *
 * A ne pas confondre avec nudge-missing-photo, qui porte sur avatar_url : deux
 * populations distinctes, deux effets distincts, les deux nudges coexistent.
 *
 * Deux modes :
 *   - { mode: "count" } : renvoie juste le nombre de destinataires eligibles
 *   - { mode: "send" }  : envoie les emails (defaut)
 *
 * Audience : RPC public.sitters_awaiting_gallery_photo (service_role only).
 * Dedup : email_send_log, template_name = 'nudge-sitter-gallery-photo',
...
const TEMPLATE_NAME = "nudge-sitter-gallery-photo";
const RUN_LIMIT = 150;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Audience éligible : regle d'indexabilite, cf. fonction SQL.
    const { data: profiles, error: pErr } = await service
      .rpc("sitters_awaiting_gallery_photo", { p_limit: RUN_LIMIT * 4 });
      ...
      try {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/send-transactional-email`, {
          ...
          body: JSON.stringify({
            ...
            idempotencyKey: `nudge-sitter-gallery-photo-${t.id}`,
    ...
    console.error("[nudge-sitter-gallery-photo]", err);
