...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  // Send directly via Resend (same pipeline as transactional emails)
  ...
  const subject = EMAIL_SUBJECTS[emailType] || 'Notification — Guardiens'
  ...
  try {
    ...
    await supabase.from('email_send_log').insert({
      ...
      status: 'sent',
      metadata: { resend_id: resendResult.id, run_id, pipeline: 'resend_direct' },
    })
    ...
    await supabase.from('email_send_log').insert({
      ...
      metadata: { run_id, pipeline: 'resend_direct' },
    })
