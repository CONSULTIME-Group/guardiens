DROP VIEW IF EXISTS public.email_pipeline_health;
...
BEGIN
  ...
  FROM public.email_deferred_queue q
  WHERE q.status = 'abandoned' AND q.updated_at >= now() - interval '24 hours';
...
REVOKE ALL ON FUNCTION public.get_email_pipeline_health(text[]) FROM public;
GRANT EXECUTE ON FUNCTION public.get_email_pipeline_health(text[]) TO authenticated, service_role;

CREATE VIEW public.email_pipeline_health AS
  SELECT * FROM public.get_email_pipeline_health();

GRANT SELECT ON public.email_pipeline_health TO authenticated, service_role;
