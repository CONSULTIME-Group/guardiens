...
BEGIN
  ...
  ELSIF p_context_type IN ('sitter_inquiry', 'helper_inquiry') THEN
    v_owner_id := v_me;
  ...
  IF p_sit_id IS NOT NULL THEN
    ...
    -- plutot que de couper la discussion en deux (meme orientation uniquement).
  ...
  ELSE
    -- Deduplication par interlocuteur, dans les DEUX sens : chaque membre est
    -- a la fois proprietaire et gardien, le couple n'est donc pas oriente.
    -- Un fil trouve en sens inverse est retourne tel quel (participants immuables).
    -- Priorite : annonce, puis sans contexte, puis mission. A rang egal, le plus recent.
    SELECT id INTO v_conv_id FROM conversations
    WHERE ((owner_id = v_owner_id AND sitter_id = v_sitter_id)
        OR (owner_id = v_sitter_id AND sitter_id = v_owner_id))
      AND context_type IS NOT NULL
    ORDER BY
      CASE
        WHEN sit_id IS NOT NULL THEN 0
        WHEN small_mission_id IS NULL THEN 1
        ELSE 2
      END,
