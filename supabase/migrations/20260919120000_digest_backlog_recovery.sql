...
-- Preserve existing rules and permissions; daily admin cron applies the new rule.
BEGIN;
...
BEGIN
  IF md5(pg_get_functiondef('public.auto_resolve_admin_signals()'::regprocedure))
      <> '6a340c11098280731b8e38d80bff486a' THEN
    RAISE EXCEPTION 'auto_resolve_admin_signals changed since review; rebase required';
  END IF;
  IF has_function_privilege('anon', 'public.auto_resolve_admin_signals()', 'EXECUTE')
    ...
    RAISE EXCEPTION 'Unexpected auto_resolve_admin_signals permissions; review required';
  END IF;
END;
$guard$;
