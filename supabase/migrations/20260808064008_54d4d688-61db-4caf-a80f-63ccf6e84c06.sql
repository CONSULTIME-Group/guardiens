...
BEGIN
  ...
  -- Une chaine de reports n'est anormale que si elle dure. Un gabarit reporte
  -- par le plafond quotidien atteint legitimement 3 tentatives avant que la
  -- duree de vie du gabarit ne tranche (envoi immediat ou annulation). On ne
  -- compte donc que les lignes enfilees depuis plus de 24 heures.
  SELECT
    COUNT(*),
    ...
    COUNT(*) FILTER (
      WHERE q.attempts >= 3
        AND q.first_enqueued_at < now() - interval '24 hours'
    ),
...
REVOKE ALL ON FUNCTION public.get_email_pipeline_health(text[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_email_pipeline_health(text[]) TO service_role;