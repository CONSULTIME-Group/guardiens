-- Lot 1 "vérité utilisateur" : véhicule persisté, durée min alignée, country jamais vide.

-- D1 : le champ véhicule du formulaire Mobilité n'avait ni colonne ni sortie.
ALTER TABLE public.sitter_profiles ADD COLUMN IF NOT EXISTS vehicle_type text;
COMMENT ON COLUMN public.sitter_profiles.vehicle_type IS 'Type de véhicule déclaré au formulaire Mobilité (car, motorcycle, transit, bike). NULL si non renseigné.';
COMMENT ON COLUMN public.sitter_profiles.min_duration IS 'Colonne héritée, non utilisée. La durée minimum saisie et affichée est min_stay_duration.';

-- D3 : un pays vide ne doit jamais être écrit. Normalisation de l'existant,
-- puis parité client/serveur : '' est traité comme inconnu dans les deux scores.
CREATE TEMP TABLE _country_fix_ids AS SELECT id FROM public.profiles WHERE country = '';
UPDATE public.profiles SET country = NULL WHERE id IN (SELECT id FROM _country_fix_ids);
...
BEGIN
  ...
  -- Parité client : un country vide ('') vaut "inconnu", donc règle France.
  v_location_ok := (
...
-- Recalcul des scores des profils normalisés.
DO $$
DECLARE
  r record;
BEGIN
  FOR r IN SELECT id FROM _country_fix_ids LOOP
    PERFORM public.calculate_profile_completion(r.id);
  END LOOP;
END $$;
DROP TABLE _country_fix_ids;

-- D2 : la fiche publique lit la colonne réellement saisie (min_stay_duration)
-- et expose le véhicule. min_duration (héritée) sort de la vue.
...
SELECT s.user_id, s.motivation, s.sitter_type, s.accompanied_by, s.lifestyle, s.animal_types,
       s.has_vehicle, s.vehicle_type, s.geographic_radius, s.min_stay_duration, s.is_available, s.competences,
