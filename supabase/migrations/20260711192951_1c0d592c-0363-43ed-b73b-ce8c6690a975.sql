...
BEGIN
  ...
  -- Missions ouvertes avec mention d'argent
  WITH m AS (
  ...
  -- Vérifications identité en attente
  ...
  IF v_health IS NOT NULL THEN
    IF v_health.last_run_age_seconds IS NULL OR v_health.last_run_age_seconds > 600 THEN
      v_pipeline_critical := true;
      v_pipeline_reasons := v_pipeline_reasons || to_jsonb('Worker silencieux (' || COALESCE(v_health.last_run_age_seconds::text,'jamais') || 's)');
    END IF;
  ...
  -- Annonces publiées sans candidature depuis > 7 jours
  WITH s AS (
  ...
  -- Gardes en retard (confirmed dont end_date < aujourd'hui)
  WITH s AS (
  ...
  -- Nouveaux 7 derniers jours, profil incomplet
