...
BEGIN
  ...
  -- Ferme uniquement les alertes dont le défaut a disparu.
  UPDATE content_quality_alerts a
     SET resolved_at = now()
   WHERE a.resolved_at IS NULL
     AND NOT EXISTS (
       SELECT 1 FROM v_content_defects v
  ...
  -- Ouvre uniquement les défauts qui ne sont pas déjà suivis.
