...
DECLARE
  ...
  v_degels int; v_degels_details jsonb;
BEGIN
  ...
  -- Gels arrives a echeance dont la page porte encore des alertes ouvertes.
  WITH degels AS (
    SELECT f.slug, f.source_table, f.frozen_until,
           count(a.id) AS alertes_ouvertes
      FROM content_freeze f
      JOIN content_quality_alerts a
        ON a.resolved_at IS NULL
       AND a.slug IS NOT DISTINCT FROM f.slug
       AND a.source_table IS NOT DISTINCT FROM f.source_table
     WHERE f.frozen_until < current_date
     GROUP BY f.slug, f.source_table, f.frozen_until
  )
  SELECT count(*),
         coalesce(jsonb_agg(jsonb_build_object(
           'cible', slug, 'table', source_table,
           'gel_echu_le', frozen_until, 'alertes_ouvertes', alertes_ouvertes)), '[]'::jsonb)
    INTO v_degels, v_degels_details
  FROM degels;
  ...
  IF v_degels > 0 THEN
    INSERT INTO admin_signals(signal_type, severity, entity_type, entity_id, metadata)
    VALUES ('content_freeze_expired', 'warning', 'content', v_sys,
      jsonb_build_object('nombre', v_degels, 'details', v_degels_details))
    ON CONFLICT (signal_type, entity_id) WHERE resolved_at IS NULL
    DO UPDATE SET metadata = EXCLUDED.metadata, severity = EXCLUDED.severity;
    v_signaux := v_signaux || jsonb_build_array('content_freeze_expired');
  END IF;
