...
DECLARE
  _role text;
BEGIN
  -- Only run when user confirms email (email_confirmed_at changes from null to a value)
...
DROP TRIGGER IF EXISTS on_email_confirmed_apply_role ON auth.users;
CREATE TRIGGER on_email_confirmed_apply_role
  AFTER UPDATE ON auth.users
  FOR EACH ROW
  EXECUTE FUNCTION public.apply_role_from_metadata();
