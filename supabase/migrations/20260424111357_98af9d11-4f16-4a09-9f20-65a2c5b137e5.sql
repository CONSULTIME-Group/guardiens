...
BEGIN
  ...
  ELSE
    -- SITTER scoring (also for 'both' role) - 100 points total
    -- Prénom + localisation: 15 pts
    IF v_first_name IS NOT NULL AND v_first_name != '' AND v_postal_code IS NOT NULL AND v_postal_code != '' THEN
      v_score := v_score + 15;
    END IF;
    ...
    -- Au moins 1 photo galerie: 5 pts
    SELECT EXISTS(
...
-- Recalculate all existing profiles with the new scale
DO $$
DECLARE
  r record;
BEGIN
