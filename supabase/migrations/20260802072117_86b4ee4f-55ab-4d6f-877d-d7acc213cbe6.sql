...
DROP FUNCTION IF EXISTS public.get_email_pipeline_health();
...
CREATE OR REPLACE FUNCTION public.get_email_pipeline_health()
 RETURNS TABLE(
   ...
   attempts_1h bigint,
   deferred_pending_total bigint,
   deferred_pending_over_2h bigint,
   deferred_pending_over_24h bigint,
   deferred_attempts_ge_3 bigint,
   deferred_oldest_age_seconds numeric,
   deferred_expired_24h bigint,
   deferred_stale_rows jsonb
 )
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    COUNT(*),
    COUNT(*) FILTER (WHERE q.first_enqueued_at < now() - interval '2 hours'),
    COUNT(*) FILTER (WHERE q.first_enqueued_at < now() - interval '24 hours'),
    COUNT(*) FILTER (WHERE q.attempts >= 3),
    MIN(q.first_enqueued_at)
  INTO deferred_pending_total, deferred_pending_over_2h, deferred_pending_over_24h,
       deferred_attempts_ge_3, _def_oldest
  ...
  FROM public.email_deferred_queue q
  WHERE q.status = 'expired' AND q.updated_at >= now() - interval '24 hours';

  -- Detail des lignes en attente depuis plus de 2h : le tri transactionnel
  -- se fait cote watchdog, qui connait la liste de _shared/email-categories.ts
  SELECT COALESCE(jsonb_agg(x ORDER BY x->>'first_enqueued_at'), '[]'::jsonb)
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'age_seconds', round(EXTRACT(EPOCH FROM (now() - q.first_enqueued_at)))
    ) AS x
    FROM public.email_deferred_queue q
    WHERE q.status = 'pending'
      AND q.first_enqueued_at < now() - interval '2 hours'
    ORDER BY q.first_enqueued_at ASC
    LIMIT 50
  ) s;
...
CREATE VIEW public.v_email_pipeline_health AS
