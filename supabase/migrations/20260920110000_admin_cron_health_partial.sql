...
DO $guard$
BEGIN
  IF md5(pg_get_functiondef('public.admin_cron_health()'::regprocedure)) <> '3b0c62bbbaf173b7faa6c02d92b38ff7' THEN
    RAISE EXCEPTION 'admin_cron_health definition changed; re-review required';
  END IF;
END; $guard$;
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'partial_7d', COALESCE(rs.partial_7d, 0),
    ...
    END
      ...
      WHEN lr.started_at IS NULL
           OR lr.started_at < now() - (c.expected_interval_min * 2 || ' minutes')::interval
        THEN 0
      WHEN COALESCE(rs.failed_7d, 0) >= 1 OR COALESCE(rs.partial_7d, 0) >= 1 THEN 1
      ELSE 2
