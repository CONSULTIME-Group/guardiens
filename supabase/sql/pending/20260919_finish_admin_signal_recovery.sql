...
-- No reconciliation call: the existing natural admin cron applies these rules.
BEGIN;
...
BEGIN
  IF md5(pg_get_functiondef('public.auto_resolve_admin_signals()'::regprocedure))
      <> '960e3c2f55893df7b29b409f1cc31fb4' THEN
...
BEGIN
  ...
  WITH candidates AS (
    ...
    FROM (SELECT * FROM candidates UNION ALL SELECT * FROM expired
          ...
          UNION ALL SELECT * FROM city_coverage_recovered) c
    WHERE s.id = c.id
