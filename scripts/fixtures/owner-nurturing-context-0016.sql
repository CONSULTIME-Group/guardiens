-- Copie de lecture de la migration 0016, NE PAS REJOUER en production.
--
-- La migration appliquée est drizzle/migrations/0016_owner_nurturing_context.sql,
-- posée par l'outil de migration. Ce fichier sert au test SQL
-- scripts/test-owner-nurturing-context.mjs et à la lecture du dépôt.
--
-- Lot N2, 22/09/2026 : le compteur propriétaire couvre tous les gardiens actifs
-- à moins de 30 km, sans seuil de complétion, d'identité ou d'activité récente.
-- La fonction renvoie aussi les trois profils prioritaires. Elle calcule des
-- distances à partir des coordonnées exactes : son exécution est réservée au
-- service_role, les appelants sont evaluate-journeys et send-onboarding-j1.
...
COMMENT ON FUNCTION public.get_owner_nurturing_context(uuid) IS
  'Contexte des relances proprietaire. Compte tous les gardiens actifs a moins de 30 km et retourne les trois profils prioritaires. Execution reservee au service_role, la fonction expose des distances calculees sur des coordonnees exactes.';
