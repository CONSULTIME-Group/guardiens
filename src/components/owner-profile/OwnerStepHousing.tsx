    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Textarea value={data.description} onChange={e => onChange({ description: e.target.value })}
          placeholder="Ce qui fait le charme de votre logement, son ambiance, ses atouts..."
          className="rounded-lg min-h-[120px]" maxLength={3000} />
        <HintBubble>Imaginez que vous accueillez un ami : que lui diriez-vous de votre maison ?</HintBubble>
      </div>

      <div className="space-y-2">
        <Label>Atouts de la région</Label>
        <Textarea value={data.region_highlights} onChange={e => onChange({ region_highlights: e.target.value })}
          placeholder="Activités, balades, sites touristiques, commerces à proximité..."
          className="rounded-lg min-h-[100px]" maxLength={2000} />
        <HintBubble>Un gardien choisit autant un lieu qu'une mission. Les sentiers de rando, le marché du samedi, le lac à 10 minutes — tout compte.</HintBubble>
      </div>

      {/* Location profile generated by AI */}
      <LocationProfileCard
        city={data.city}
        postalCode={data.postal_code}
        editable
        onUseDescription={(text) => onChange({ region_highlights: text })}
      />
    </div>
  );
};
