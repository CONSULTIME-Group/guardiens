  ...
  return (
    <div className="space-y-8">
      ...
      <DashSection title="Mes animaux" action={
        ...
        {pets.length === 0 ? (
          <EmptyCard icon={PawPrint} text="Aucun animal enregistré." cta="Ajouter un animal" to="/owner-profile" />
        ) : (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard icon={Heart} iconColor="text-pink-500" label="Gardiens de confiance" value={trustedCount} delay={300} />
      </div>
            ...
            {recentApps.map(app => (
              <Link key={app.id} to={`/sits/${app.sit_id}`} className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border hover:bg-accent/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {app.sitter?.id && <BadgePills userId={app.sitter.id} max={2} size="sm" />}
