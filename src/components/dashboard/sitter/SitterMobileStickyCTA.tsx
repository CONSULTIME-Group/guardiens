...
const SitterMobileStickyCTA = memo(({ pendingAppsCount = 0, unreadCount = 0 }: SitterMobileStickyCTAProps) => {
  ...
  return (
    <div
      ...
      className={`md:hidden fixed bottom-[var(--bottom-nav-h,0px)] left-0 right-0 z-40 bg-background/95 backdrop-blur-sm border-t border-border px-4 py-3 shadow-[0_-4px_12px_-4px_hsl(var(--foreground)/0.08)] motion-safe:transition-opacity motion-safe:duration-200 ${starVisible ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      role="region"
