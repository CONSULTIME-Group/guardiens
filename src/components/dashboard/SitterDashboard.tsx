  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [appsRes, sitterRes, profileRes, reviewsRes, listingsRes, badgesRes, missionsRes, articlesRes] = await Promise.all([
        ...
        supabase.from("articles").select("id, title, slug, cover_image_url, excerpt, category").eq("published", true).in("category", ["conseil_gardien", "guide_pratique", "thematique"]).order("published_at", { ascending: false }).limit(3),
      ]);
