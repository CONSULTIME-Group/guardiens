...
const MissionRow = ({ m }: { m: SmallMission }) => {
  const responseCount = m.small_mission_responses?.length || 0;
  const isCompleted = m.status === "completed";
  return (
    <Link to={`/petites-missions/${m.id}`} className="flex items-center gap-3 py-2.5 border-b border-border last:border-0">
...
const MyMissionsColumn = memo(({ missions }: { missions: SmallMission[] }) => {
  ...
  const { active, archived } = useMemo(() => {
    const a: SmallMission[] = [];
    const ar: SmallMission[] = [];
    for (const m of missions) {
      (m.status === "completed" ? ar : a).push(m);
    }
      ...
      {missions.length === 0 ? (
        ...
        <>
          {active.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground italic py-2">Aucune mission active.</p>
          )}
          ...
          {archived.length > 0 && (
            <details className="group mt-2">
              <summary className="cursor-pointer list-none text-xs text-muted-foreground hover:text-foreground transition-colors py-2 flex items-center justify-between">
                ...
                <span className="text-muted-foreground group-open:rotate-180 transition-transform" aria-hidden="true">▾</span>
              </summary>
              <div className="pt-1">
                {archived.map(m => <MissionRow key={m.id} m={m} />)}
              </div>
            </details>
          )}
        </>
