...
const StickyCTA = ({ city, stats }: Props) => {
  ...
  if (isBeforeLaunch && stats.guardiansCount > 0) {
    message = `${stats.guardiansCount} gardien${stats.guardiansCount > 1 ? "s" : ""} déjà inscrit${stats.guardiansCount > 1 ? "s" : ""} à ${city.name}. Rejoignez les membres fondateurs avant le 14 juillet.`;
