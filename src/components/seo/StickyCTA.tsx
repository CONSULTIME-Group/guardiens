...
const StickyCTA = ({ city, stats }: Props) => {
  ...
  if (stats.guardiansCount > 0 && stats.activeListings === 0) {
    message = `${stats.guardiansCount} gardien${stats.guardiansCount > 1 ? "s" : ""} déjà inscrit${stats.guardiansCount > 1 ? "s" : ""} à ${city.name}. Rejoignez la communauté, pendant la phase de lancement.`;
