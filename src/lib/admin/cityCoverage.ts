 ...
 * s'affiche à côté, comme information.
 *
...
/** Fragment « N gardiens à 30 km », toujours chiffré, jamais « 0 gardien » à tort. */
export const buildSittersFragment = (m: CityCoverageMetrics): string =>
  m.sittersCount === 0
    ? `aucun gardien à ${m.radiusKm} km`
    : `${m.sittersCount} ${plural(m.sittersCount, "gardien", "gardiens")} à ${m.radiusKm} km`;
...
export const buildVerifiedFragment = (m: CityCoverageMetrics): string => {
  ...
  if (m.verifiedSittersCount === 0) return "aucun n'a encore vérifié son identité";
  return `dont ${m.verifiedSittersCount} ${plural(
    m.verifiedSittersCount,
    "avec identité vérifiée",
    "avec identité vérifiée",
  )}`;
};
...
export const buildCoverageGapMessage = (m: CityCoverageMetrics): string => {
  const parts = [`${m.city} compte ${buildSittersFragment(m)}`];
  const verified = buildVerifiedFragment(m);
  if (verified) parts.push(verified);
  return `${parts.join(", ")}. Cibler la ville en recrutement.`;
};
...
export const buildSeoTensionMessage = (
  ...
  `${m.city} génère ${m.impressions} impressions Google pour ${buildSittersFragment(
    m,
  )}. La demande arrive plus vite que l'offre.`;
