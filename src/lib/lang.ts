 ...
 * i18next (lookupLocalStorage). Depuis le 17/08/2026, le français est la
 * seule langue supportée : cette mémoire ne peut donc contenir que « fr ».
 */
 ...
 *
 * Guardiens est monolingue français depuis le 17/08/2026 : toute valeur
 * `?lang=` non supportée (en, de, it, es ou autre) impose le français
 * immédiatement, sans tenir compte du choix mémorisé ni du navigateur. C'est
 * ce qui garantit qu'une ancienne URL `?lang=en` connue de Google rend la
 * page française indexable avec sa canonique auto-référente.
 */
