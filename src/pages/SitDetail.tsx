...
const SitDetail = () => {
  ...
  const tabItems = [
    ...(isOwner ? [{ value: "candidatures", label: `Candidatures (${appCount})`, hasDot: pendingAppCount > 0 }] : []),
    { value: "animals", label: "🐾 Animaux" },
    { value: "housing", label: "🏠 Logement" },
    { value: "expectations", label: "📋 Attentes" },
    ...
    <div className="space-y-6">
      ...
      {photos.length > 0 && (
        <div className="relative">
          <img src={photos[photoIndex]} alt="Logement" className="w-full h-40 rounded-xl object-cover" />
          {photos.length > 1 && (
            <div className="flex gap-1.5 mt-2 overflow-x-auto pb-1">
              {photos.map((p: string, i: number) => (
                <button
                  ...
                  className={`w-12 h-9 rounded-md object-cover border-2 shrink-0 overflow-hidden transition-all ${i === photoIndex ? "border-primary ring-2 ring-primary/30" : "border-transparent opacity-70 hover:opacity-100"}`}
                >
      ...
      {/* Title + status */}
      <div>
        <h1 className="font-heading text-lg font-bold">{sit.title || `Garde à ${owner?.city || "..."}`}</h1>
        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground mt-2">
          {owner?.city && (
            <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{owner.city}</span>
          )}
        ...
        {avgRating && (
          <span className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
            <Star className="h-3 w-3 text-secondary fill-secondary" />{avgRating} ({reviews.length})
          </span>
        )}
      </div>
      ...
      {owner && (
        <div className="flex items-center gap-3 p-3 bg-card rounded-xl border border-border">
          <Link to={`/profil/${owner.id}`}>
            {owner.avatar_url ? (
              <img src={owner.avatar_url} alt={owner.first_name} className="w-10 h-10 rounded-full object-cover" />
            ) : (
          ...
          <div className="flex-1 min-w-0">
            <Link to={`/profil/${owner.id}`} className="text-sm font-medium flex items-center gap-1 hover:underline">
              {owner.first_name}
      ...
      <div className="flex flex-col gap-2">
        ...
        {isOwner && (
          <Link
            to={`/annonces/${sit.id}`}
            target="_blank"
            className="text-xs text-primary hover:underline cursor-pointer flex items-center gap-1 justify-center"
          >
      ...
      {/* Tab navigation */}
      <div className="flex flex-row md:flex-col gap-1 overflow-x-auto md:overflow-x-visible scrollbar-hide border-t border-border pt-4">
        {tabItems.map((tab) => (
          <button
            key={tab.value}
            onClick={() => setActiveTab(tab.value)}
            className={`px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors flex items-center gap-2 w-full text-left ${
              activeTab === tab.value
          ...
          >
            {tab.label}
            {tab.hasDot && <span className="w-2 h-2 rounded-full bg-primary-foreground inline-block" />}
          </button>
        ))}
      </div>
    </div>
  ...
  const rightContent = (
    <div className="pb-32 space-y-6">
      {/* Draft banner */}
      {isOwner && isDraft && (
        <div className="rounded-xl border border-border bg-accent/50 p-4 md:p-5">
      ...
      {/* Tab content */}
      {activeTab === "candidatures" && isOwner && (
        <ApplicationsList
          sitId={sit.id}
          sitTitle={sit.title}
