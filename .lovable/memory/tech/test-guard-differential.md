...
- `scripts/test-guard/baseline.json` : 18 échecs connus tolérés (raison documentée pour chacun ; les deux `it()` homonymes de `notify-mission-event-invocations` fusionnent en un seul identifiant, d'où 18 et non 19) + 2 fichiers exclus du périmètre (`article-internal-links`, `article-pricing-content`, dépendants de la base de production).
